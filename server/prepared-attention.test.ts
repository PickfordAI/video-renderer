...
describe('prepared stationary attention', () => {
  it('uses a validated respondent and makes visible listeners watch the speaker', () => {
    ...
    expect(prompt).toContain("speaking to Bea with their gaze on Bea's face");
    expect(prompt).toContain('Cy preserves the head angle and eye direction shown in Image 1 throughout the shot, watching Ada');
  ...
  it('authored look overrides derived speaker and listener gaze', () => {
    ...
    expect(prompt).not.toContain('speaking to Bea');
    expect(prompt).not.toContain('Ada preserves the head angle');
    expect(prompt).not.toContain('Bea preserves the head angle');
    expect(prompt).not.toContain('watching Ada');
  });
});
