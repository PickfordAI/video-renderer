...
async function fixture() {
  ...
  const acknowledgements = () => events.filter(event => event.type === 'renderer.event.verdict.ack');
  return {
...
describe('StoryKernel completion verdict handshake', () => {
  it('finishes accepted playback before the verdict deadline, then writes the exact ACK before natural close', async () => {
    ...
    try {
      ...
      expect(test.run.eventVerdicts).toMatchObject({ pending: 1, acknowledged: 0 });
      expect(test.stop).not.toHaveBeenCalled();
      ...
      expect(test.run.eventVerdicts).toMatchObject({ pending: 0, acknowledged: 1, refused: 0 });
  ...
  it('ACKs a negative authoritative verdict after recording refusal, then fails the run', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(test.run.state).toBe('failed'));
      expect(test.run.eventVerdicts).toMatchObject({ acknowledged: 1, pending: 0, refused: 1, lastOutcome: { outcome: 'behind_confirmed_frontier' } });
  ...
  it('reports transport rejection without sending a nonexistent verdict ACK', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => expect(test.run.state).toBe('failed'));
      expect(test.run.eventVerdicts).toMatchObject({ acknowledged: 0, pending: 1, refused: 1, lastTransportRejection: { code: 'idempotency_conflict' } });
      expect(test.acknowledgements()).toEqual([]);
    } finally { await test.close(); }
  });
