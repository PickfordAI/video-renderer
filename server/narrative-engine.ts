...
export async function handleNarrativeEngineApi(
  ...
  if (!isRoom && !isStartShow && !isProvisionExternalStory && !isStopShow && !isExternalPlayback && !isAvailableEvds && !isDss) return false;

  try {
    if (isRoom || isStartShow || isProvisionExternalStory || isStopShow || isExternalPlayback || isAvailableEvds) {
